...
export async function sessionServicePaper(session) {
    try {
        ...
        if (!isSession)
            return { ok: false, data: { message: sessionUnexist, reason: "no session" } };
        ...
        if (isSession.revoked)
            return { ok: false, data: { message: sessionUnexist, reason: "session revoked" } };
        ...
        if (isSession.sessionElapsed < new Date()) {
            ...
            return { ok: false, data: { message: sessionUnexist, reason: "revoked" } };
        }
        ;
